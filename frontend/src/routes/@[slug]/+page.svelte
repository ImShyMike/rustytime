<script lang="ts">
	...
	onMount(() => {
		requestAnimationFrame(() => {
			if (data.username && page.params.slug !== data.username) {
				replaceState(`/@${data.username}`, {});
			}
		});
	});
...
<PageScaffold title="@{data.username}" showLastUpdated={false}>
	...
	<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
		<StatCard title="Today" value={formatDuration(data.time.today, false)} />
		<StatCard title="This Week" value={formatDuration(data.time.week, false)} />
		<StatCard title="All Time" value={formatDuration(data.time.all_time, false)} />
	</div>

	<!-- Projects -->
	<SectionTitle className="mb-3"
		>Top Projects <span class="text-sm text-subtext0">(Past Month)</span></SectionTitle
	>
